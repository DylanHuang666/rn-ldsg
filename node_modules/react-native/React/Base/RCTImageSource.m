...
- (instancetype)initWithURLRequest:(NSURLRequest *)request size:(CGSize)size scale:(CGFloat)scale extraData:(int)extraData
{
  if ((self = [super init])) {
...
- (instancetype)imageSourceWithSize:(CGSize)size scale:(CGFloat)scale extraData:(int)extraData
{
...
- (BOOL)isEqual:(RCTImageSource *)object
{
  ...
  return _extraData == object.extraData && [_request isEqual:object.request] && _scale == object.scale &&
  (CGSizeEqualToSize(_size, object.size) || CGSizeEqualToSize(object.size, CGSizeZero));
}

- (NSString *)description
{
  return [NSString stringWithFormat:@"<RCTImageSource: %p URL=%@, size=%@, scale=%0.f, extraData=%i>",
          self, _request.URL, NSStringFromCGSize(_size), _scale, _extraData];
}
...
+ (RCTImageSource *)RCTImageSource:(id)json
{
  ...
  if ([json isKindOfClass:[NSDictionary class]]) {
    ...
    extraData = [self NSInteger:json[@"extraData"]];

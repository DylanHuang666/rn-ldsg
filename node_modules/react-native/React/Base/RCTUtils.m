...
//  return nil;
  ...
  for(NSUInteger i = windows.count - 1; i >= 0; i--){
    ...
    if (window.windowLevel == UIWindowLevelNormal && !window.isHidden && [[window subviews] count] > 0){
      UIView *frontView =  [window subviews] && [window subviews].count > 0 ? [[window subviews] objectAtIndex:0] : nil;
      id nextResponder =  frontView ? [frontView nextResponder] : nil;

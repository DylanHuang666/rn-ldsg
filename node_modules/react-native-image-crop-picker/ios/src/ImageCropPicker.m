        ...
        if (exportSession.status == AVAssetExportSessionStatusCompleted) {
            ...
            [outputURL getResourceValue:&fileSizeValue
                                 forKey:NSURLFileSizeKey
                                  error:nil];
            CMTime time = compressedAsset.duration;
            long duration = ceil(time.value / time.timescale) * 1000;
...
RCT_EXPORT_METHOD(saveMp4Bitmap:(NSString*)path
                  savePath:(NSString*)savePath
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    //确保保存路径所在的文件夹已经创建了
    if (![FileUtils makeFileFolder:savePath]) {
        resolve(@(NO));
        return;
    }
    
    //获取第一帧
    AVAsset* assets = [AVAsset assetWithURL:[NSURL fileURLWithPath:path]];
    AVAssetImageGenerator* generate = [[AVAssetImageGenerator alloc] initWithAsset:assets];
    generate.appliesPreferredTrackTransform = YES;
    NSError* error = nil;
    CMTime time = CMTimeMake(1, 2);
    CGImageRef oneRef = [generate copyCGImageAtTime:time actualTime:nil error:&error];
    ...
    BOOL ret = YES;
    CGImageDestinationAddImage(destination, oneRef, nil);
